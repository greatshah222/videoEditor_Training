...
module.exports = {
  ...
  getItemAtTime(document, track, time) {
    ...
    for (let entry of entries) {
      // Blank
      if (entry.tagName === 'blank') {
        currentTime = addDuration(currentTime, entry.getAttribute('length'));
      }
      // Simple entry
      else if (new RegExp(/^producer/).test(entry.getAttribute('producer'))) {
        currentTime = addDuration(
          currentTime,
  ...
  getDuration(element, document) {
    ...
    if (element.tagName === 'multitrack') {
      ...
      duration.time = subDuration(duration.out, duration.in);
    }
    return duration;
  },
  ...
  appendPlaylistToMultitrack(
    ...
    transitionElement.setAttribute('out', addDuration(duration, overlapping));
  ...
  getItemInRange(track, timeStart, timeEnd, document) {
    ...
    for (let entry of entries) {
      // Blank
      if (entry.tagName === 'blank') {
        time = addDuration(time, entry.getAttribute('length'));
      }
      // Simple entry
      else if (new RegExp(/^producer/).test(entry.getAttribute('producer'))) {
        time = addDuration(time, this.getDuration(entry, document).time);
      }
      // Container of entries
      else {
        const tractor = document.getElementById(entry.getAttribute('producer'));
        const multitrack = tractor.getElementsByTagName('multitrack').item(0);
        time = addDuration(time, this.getDuration(multitrack, document).time);
      }
